		...
		res.each do |line| 											#looping through the lines in the file
    		...
    		if line[1].to_i > prev_index.to_i then					#When the index is changed/incremented the lines with the previous index are inserted into the LaroratoryTestResult class
  			...
  			if line[0] == 'OBX' 									#we pick the code and result from the line starting with OBX
  				...
  				if comment != '' then comment = comment + "\n" end		#append \n if multiple comments are present
...
#Tested for below scenarios : 
#1. If multiple comments are present for one code they should be attached to same line.
#2. The number of index should equal to number of objects created.
#3. Correct format and result should be parsed.
#4. Print message for when the file is empty.